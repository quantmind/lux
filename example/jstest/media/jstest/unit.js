...
test("logger", function() {
    ...
    test("Model", function () {
        ...
        ok(book.cid(), '... it has an cid value');
        ...
        //
        // Test update
        book = new Book({'id': 14, 'pages': 300, 'author': 'luca'});
        ok(book.pk(), '... it has an pk value');
        equal(book.cid(), 'book-14');
        book.set('pages', 300);
        equal(_.size(book.changedFields()), 0, '... no fields have changed');
        book.set('pages', 250);
        equal(_.size(book.changedFields()), 1, '... 1 field has changed');
        equal(book.previous('pages'), 300);
        //
        var data = book.toJSON(true);
        equal(_.size(data), 1);
        ok(_.isEqual(data, {'pages': 250}));
        //
